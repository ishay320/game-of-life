...
def get_neighbors(board: list[list[str]], y: int, x: int) -> int:
    ...
    for i in range(y - 1, y + 2):
        for j in range(x - 1, x + 2):
            if board[(i + height) % height][(j + width) % width] == states[1] and not (
                i == y and j == x
            ):
                count += 1
    return count
...
def step(board_a: list[list[str]], board_b: list[list[str]]) -> None:
    for i in range(height):
        for j in range(width):
            ...
            if board_a[i][j] == states[1] and (neighbors == 2 or 3 == neighbors):
...
if __name__ == "__main__":
    board_a: list[list[str]] = [
        [states[0] for i in range(width)] for j in range(height)
    ]
    board_b: list[list[str]] = [
        [states[0] for i in range(width)] for j in range(height)
    ]
